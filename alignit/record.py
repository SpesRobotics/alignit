...
def generate_spiral_trajectory(
    start_pose, # This will now be the absolute starting point of the gripper
    z_step=0.001,
    radius_step=0.001,
    num_steps=100,
    cone_angle=45,
    ...
    lift_height_before_spiral=0.1 # New parameter: how high to lift from start_pose before spiraling
):
    """
    Generate spiral trajectory that maintains the initial object rotation throughout,
    with an initial straight lift-up phase from the start_pose.

    Args:
        start_pose: Initial pose of the gripper (the lowest point of the lift).
        z_step: Step size in z-axis (depth) for the spiral.
        ...
        include_cone_poses: Whether to include viewing cone poses for the spiral part.
        initial_lift_steps: Number of poses to generate for the initial lift-up.
        lift_height_before_spiral: The total height to lift straight up from start_pose
                                   before the spiral motion begins.
    """
    trajectory = []
    R_start = start_pose[:3, :3]  # This maintains the object's initial rotation
    t_start_initial = start_pose[:3, 3] # This is the absolute starting position

    cone_angle_rad = np.deg2rad(cone_angle)

    # --- First Loop: Generate initial straight lift-up poses ---
    # The lift-up starts at t_start_initial and goes straight up by lift_height_before_spiral
    if initial_lift_steps > 0:
        # Calculate the Z-increment for each lift-up step
        # Ensure division by zero is handled if initial_lift_steps is 1
        ...
        for j in range(initial_lift_steps):
            # Calculate the current Z-position for the lift-up
            current_z_lift = t_start_initial[2] + (j * z_increment_per_step)

            # Create the position for the lift-up pose (X, Y remain constant)
            lift_position = np.array([t_start_initial[0], t_start_initial[1], current_z_lift])

            # Create the pose with initial rotation and the new lift-up position
            T_lift = np.eye(4)
            T_lift[:3, :3] = R_start  # Maintain the same rotation
            T_lift[:3, 3] = lift_position
            trajectory.append(T_lift)

    # The actual start position for the spiral will be the end of the lift
    t_start_spiral = np.array([t_start_initial[0], t_start_initial[1], t_start_initial[2] + lift_height_before_spiral])


    # --- Second Loop: Generate spiral motion and optional cone poses ---
    # Calculate angular range with offset for cone poses
    ...
    for i in range(num_steps):
        ...
        # Calculate position in local frame relative to t_start_spiral
        local_offset = np.array([
            radius * np.cos(angle),
            radius * np.sin(angle),
            -z_step * i # Z decreases from the spiral start point
        ])

        # Transform to world frame using initial rotation
        world_offset = R_start @ local_offset
        base_position = t_start_spiral + world_offset # Spiral starts from the lifted height
...
def main():
    ...
    for episode in range(20):
        ...
        roll = np.pi  
        pitch =0 # np.random.uniform(0 ,np.pi/4)   
        yaw = 0 #np.random.uniform(-np.pi/2, np.pi/2)        
        ...
        trajectory = generate_spiral_trajectory(
            pose_start,
            z_step=0.0005,        # Increased from 0.0005
