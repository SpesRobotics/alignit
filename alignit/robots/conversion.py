...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-o",
        "--output_dir", # Renamed to output_dir
        type=str,
        required=True,
        help="output directory for the decomposed convex hull parts.",
    )
    ...
    parser.add_argument(
        ...
        "--max-convex-hull",
        type=int,
        default=-1, # Changed from -1 to allow limiting
    ...
    for i, (vs, fs) in enumerate(result):
        ...
        output_filepath = os.path.join(output_dir, f"stator_part_{i:03d}.stl")
        part_mesh.export(output_filepath)
        mesh_parts.append(part_mesh) # Keep track if you want to inspect them together later

    print(f"Generated {len(mesh_parts)} convex hull parts.")
