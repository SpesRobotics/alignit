...
class DatasetConfig:
    ...
    hf_username: Optional[str] = field(default=None, metadata={"help": "Hugging Face username for dataset sync"})
    hf_dataset_name: Optional[str] = field(default=None, metadata={"help": "Hugging Face dataset name for sync"})
    hf_token: Optional[str] = field(default=None, metadata={"help": "Hugging Face token for authentication"})
    prefer_local: bool = field(default=True, metadata={"help": "Prefer local dataset over Hugging Face Hub"})
    push_to_hub: bool = field(default=False, metadata={"help": "Automatically push datasets to Hugging Face Hub after saving"})
...
class TrajectoryConfig:
    ...
    z_step: float = field(default=0.0007, metadata={"help": "Z step size for spiral trajectory"})
    radius_step: float = field(default=0.001, metadata={"help": "Radius step size for spiral trajectory"})
    num_steps: int = field(default=100, metadata={"help": "Number of steps in spiral trajectory"})
    cone_angle: float = field(default=30.0, metadata={"help": "Cone angle in degrees"})
    visible_sweep: float = field(default=60.0, metadata={"help": "Visible sweep angle in degrees"})
    viewing_angle_offset: float = field(default=-120.0, metadata={"help": "Viewing angle offset in degrees"})
    angular_resolution: float = field(default=10.0, metadata={"help": "Angular resolution in degrees"})
    include_cone_poses: bool = field(default=False, metadata={"help": "Include cone poses in trajectory"})
    lift_height_before_spiral: float = field(default=0.01, metadata={"help": "Lift height before spiral in meters"})
...
class RecordConfig:
    ...
    lin_tol_alignment: float = field(default=0.015, metadata={"help": "Linear tolerance for alignment servo"})
    ang_tol_alignment: float = field(default=0.015, metadata={"help": "Angular tolerance for alignment servo"})
    lin_tol_trajectory: float = field(default=0.05, metadata={"help": "Linear tolerance for trajectory servo"})
    ang_tol_trajectory: float = field(default=0.05, metadata={"help": "Angular tolerance for trajectory servo"})
...
class InferConfig:
    ...
    ang_tolerance: float = field(default=2.0, metadata={"help": "Angular tolerance for convergence (degrees)"})
    max_iterations: Optional[int] = field(default=None, metadata={"help": "Maximum iterations before stopping (None = infinite)"})
    debug_output: bool = field(default=True, metadata={"help": "Print debug information during inference"})
    debouncing_count: int = field(default=5, metadata={"help": "Number of consecutive iterations within tolerance before stopping"})
